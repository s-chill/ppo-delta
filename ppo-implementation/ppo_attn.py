...
class PPO:
    def __init__(self, env, **hyperparameters):
        ...
        # Fix for HWC image format → CHW for CNN
        if len(self.obs_shape) == 3:
            self.obs_shape = (self.obs_shape[2], self.obs_shape[0], self.obs_shape[1])

        self.act_dim = env.action_space.n if self.discrete else env.action_space.shape[0]

        self.actor = FeedForwardNN(self.obs_shape, self.act_dim, discrete=self.discrete, use_attention=True)
        self.critic = FeedForwardNN(self.obs_shape, 1, discrete=False, use_attention=True)
    ...
    def rollout(self):
        ...
        batch_obs = torch.tensor(np.array(batch_obs), dtype=torch.float32) # beware of danger
    ...
    def evaluate(self, batch_obs, batch_acts):
      ...
      if self.discrete:
          ...
          # Fix shape: ensure batch_acts is [B, act_dim]
          if batch_acts.ndim == 1:
              batch_acts = batch_acts.view(-1, self.act_dim)
